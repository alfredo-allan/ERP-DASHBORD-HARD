...
import { MdSearch } from "react-icons/md"; // Material Design Icons (Google)
import LoginModal from "../LoginModal/LoginModal"; // ✅ IMPORT DO MODAL
...
const Header: React.FC<HeaderProps> = ({
  ...
  // ✅ FUNÇÃO QUE ABRE O MODAL AO CLICAR EM CANCELAR
  const handleCancelarClick = () => {
    // Se o componente pai passou uma função, usa ela
    if (onCancelaClick) {
      onCancelaClick();
      return;
    }
    ...
    console.log("Abrindo modal de login para cancelar...");
  ...
  // ✅ FUNÇÃO QUANDO LOGIN É BEM-SUCEDIDO
  const handleLoginSuccess = () => {
    console.log("✅ Usuário autenticado! Procedendo com cancelamento...");
    setIsLoginModalOpen(false);
    // Aqui você coloca a lógica REAL de cancelamento
    alert("Cancelamento realizado com sucesso!");
    // Exemplo: fazer uma chamada API, atualizar estado, etc.
  };
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchTerm.trim()) {
      ...
      // Aqui você coloca sua lógica de busca real
    }
  };
  const SearchComponent = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      if (searchTerm.trim()) {
        ...
        console.log("Pesquisando por:", searchTerm);
        // Opcional: fechar depois de pesquisar
        ...
        <div className="max-w-full mx-auto px-4">
          ...
          <div className="flex items-center justify-between h-[42px]">
            ...
            <div className="flex items-center space-x-4">
              {/* Search Bar */}
              {/* Search Bar - COM MÁGICA DO SHOW/HIDE */}
              {/* Search Bar - ÍCONE DENTRO DO INPUT, EXPANSÃO DA DIREITA PARA ESQUERDA */}
              <div className="hidden md:flex items-center justify-end">
                <div className="relative">
                      ...
                      <form onSubmit={handleSearchSubmit} className="relative">
                        <input
                          ...
                          className="
                            ...
                            rounded-lg px-3 py-2 pl-10 pr-10
                        ...
                        {/* Ícone de lupa DENTRO do input (lado esquerdo) */}
                        <div className="absolute left-3 top-1/2 transform -translate-y-1/2"></div>

                        {/* Botão fechar DENTRO do input (lado direito) */}
                        <button
                          type="button"
                          onClick={() => {
                            setIsSearchOpen(false);
                  ...
                  {/* Ícone de lupa FIXO (fora do input, lado direito) */}
                  <button
                  ...
                  >
                    <MdSearch size={34} className="text-gray-500" />
                  </button>
                </div>
              </div>
      ...
      <LoginModal
        ...
        onLogin={async (username, password) => {
          ...
          return new Promise<void>((resolve, reject) => {
            setTimeout(() => {
              // Validação simples (em produção, faria chamada à API)
        ...
        title="Login"
        message="Esta operação requer autenticação. Por favor, faça login para cancelar o item."
      />
    </header>
  );
};
